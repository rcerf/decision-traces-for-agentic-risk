...
| Falsification discipline | 7 five-arm validation studies with pre-registered kill criteria; 4 produced KILL or FALSIFIED verdicts, including killing a personality-fingerprinting claim when a TF-IDF baseline matched the complex method | I retire claims when simpler baselines win or confound controls erase the effect | Negative results on my own private experiments do not prove skill on OpenAI-relevant systems |
